...
test("Has PocketBase auth widget", "foster-hope-pb.fly.dev" in HTML)
test("Has PocketBase auth UI", "pb-signin" in HTML)
...
test("Has weekly report form", 'sjalavard-weekly-report' in HTML)
test("Script block present", len(SCRIPT) > 1000, f"script length={len(SCRIPT)}")

print("\n=== 2. POCKETBASE BACKEND ===")
test("PocketBase URL defined", "foster-hope-pb.fly.dev" in SCRIPT)
test("PB_PROFILES defined", "PB_PROFILES" in SCRIPT)
test("PB_SESSIONS defined", "PB_SESSIONS" in SCRIPT)
test("_pbReadProfile defined", "_pbReadProfile" in SCRIPT)
test("_pbWriteProfile defined", "_pbWriteProfile" in SCRIPT)
...
test("dbSetProfile defined", "async function dbSetProfile(" in SCRIPT)
test("dbGetSessions uses PocketBase", "PB_SESSIONS" in SCRIPT and "dbGetSessions" in SCRIPT)
test("dbAddSession uses PocketBase", "PB_SESSIONS" in SCRIPT and "dbAddSession" in SCRIPT)
test("dbDeleteSession uses PocketBase", "dbDeleteSession" in SCRIPT)
test("dbProfileDone checks PocketBase or localStorage",
     'dbGetProfile' in SCRIPT or 'localStorage.getItem' in SCRIPT)
test("dbProfileDone checks uid-agnostic key",
     'localStorage.getItem("sj_profile_done")' in SCRIPT)
test("Email fallback lookup", "_pbCurrentEmail" in SCRIPT)
...
test("Calls /api/gemini", '"/api/gemini"' in SCRIPT or 'sjalavard.com/api/gemini"' in SCRIPT)
...
test("Foster Hope URL", "fosterhopefamily.com" in SCRIPT)
